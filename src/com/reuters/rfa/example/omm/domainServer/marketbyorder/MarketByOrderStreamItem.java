...
import com.reuters.rfa.example.omm.domainServer.marketbyorder.MarketByOrderGenerator.OrderEntry;
...
/**
 * MarketByOrderStreamItem is a stream item for Market By Order domain.
 * 
 * This class is responsible for encoding the Open Message Model data for Market
 * By Order domain and sending refresh and update message.
 */
public class MarketByOrderStreamItem extends DataStreamItem
{

    private OMMEncoder _encoder;
    private ItemInfo _itemInfo;
    private byte[] _preencodedSummaryDataBuffer;
    private DataDefDictionary _dataDefDictionary;

    protected MarketByOrderStreamItem(DataGenerator dataGenerator, ProvDomainMgr mgr, Token token,
            OMMMsg msg)
    {
        super(mgr, token);
    ...
    private void encodeRefresh(ItemInfo itemInfo, Object[] orderEntries, boolean completed, boolean solicited)
    {
        ...
        outmsg.setMsgModelType(RDMMsgTypes.MARKET_BY_ORDER);
        ...
        _encoder.encodeMapInit(flags, OMMTypes.BUFFER, OMMTypes.FIELD_LIST, 0, (short)3426);
        if (isEncodeDataDef())
        {
            encodeDataDef();
        }
    ...
    private void encodeDataDef()
    {
        ...
        if (_dataDefDictionary == null)
        {
            ...
            dataDef.addDef((short)3429, OMMTypes.REAL); // ORDER_SIZE
                                                        // "ORDER SIZE"
            dataDef.addDef((short)212, OMMTypes.RMTES_STRING); // MKT_MKR_ID
                                                               // "MRKT MAKER ID"
        ...
        for (int i = 0; i < orderEntries.length; i++)
        {
            ...
            if (orderEntry.action == OMMMapEntry.Action.ADD)
            {
                if (isDefined)
                {
                    ...
                    _encoder.encodeString(orderEntry.makerId, OMMTypes.RMTES_STRING);
                ...
                else
                {
                    ...
                    _encoder.encodeReal(orderEntry.orderSize, OMMNumeric.EXPONENT_0);
                    _encoder.encodeFieldEntryInit((short)212, OMMTypes.RMTES_STRING);
                    _encoder.encodeString(orderEntry.makerId, OMMTypes.RMTES_STRING);
