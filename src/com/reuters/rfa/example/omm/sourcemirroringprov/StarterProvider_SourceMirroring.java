 ...
 * 
 * <p>
 * StarterProvider_SourceMirroring is a simple interactive server provider
 * example to demonstrate how to receive generic message with the
 * SourceMirroringMode.
 * </p>
 * 
 * This application is single-threaded server application.
 * SourceMirroringConsumerDemo must connect to this server to request directory
 * data and to send the consumer status generic message.
 * 
 ...
 * 
 */
public class StarterProvider_SourceMirroring implements Client
{
    private Session _session;
    protected final Handle _csListenerIntSpecHandle, _errIntSpecHandle;
    EventQueue _eventQueue;
    boolean _acceptSession;
    Handle _clientSessionHandle = null;
    MsgProviderClientSession _clientSession = null;
    OMMProvider _provider;
    OMMPool _pool;
    int _submitInterval;
    String _service = "DIRECT_FEED";
    int _acceptingConsumerStatus;

    /**
	 *
	 */
    public StarterProvider_SourceMirroring()
    {
        System.out.println("*****************************************************************************");
        System.out.println("*        Begin RFA Java StarterProvider_SourceMirroring Program             *");
        System.out.println("*****************************************************************************");
        System.out.println("Initializing Source Mirroring Provider ...");
        readConfiguration();
        Context.initialize();
    ...
    public void processEvent(Event event)
    {
        switch (event.getType())
        {
            ...
            case Event.OMM_CMD_ERROR_EVENT: // for cmd error handle
                ...
                break;
            default:
                System.out.println("OMMGenericMsgProviderDemo: unhandled event type: "
                        + event.getType());
                break;
        }
    }
    ...
    private void processActiveClientSessionEvent(OMMActiveClientSessionEvent event)
    {
        ...
        _clientSession = new MsgProviderClientSession(this, _service);
    ...
    static void addCommandLineOptions()
    {
        ...
        CommandLine.addOption("acceptingConsumerStatus", 1,
                              "If set to 1 the flag will be set in the directory rsp");
    }
    ...
    public static void main(String argv[])
    {
        ...
        StarterProvider_SourceMirroring demo = new StarterProvider_SourceMirroring();
        demo._acceptingConsumerStatus = CommandLine.intVariable("acceptingConsumerStatus");
